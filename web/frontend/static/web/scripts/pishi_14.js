(function () {
     ...
     * Profile FUNCTIONS
     */
    $(document.body).on('change', '#profile-image-upload', function () {
       let form = $('#profile-image-upload');
       if($(form).find('input:file').val()) {
           form.submit();
       }
    });

    $(document.body).on('click', '.profile-past-item', function () {
    ...
    $(document.body).on('click', '.nav-item', function () {
        let split = $(this).prop('href').split('#');
        let activeId = split[split.length-1];
        $(this).siblings().each(function () {
            let split = $(this).prop('href').split('#');
            let inactive_id = split[split.length-1];
            $('#'+inactive_id).hide();
        });
        $('#'+activeId).show();
    });
